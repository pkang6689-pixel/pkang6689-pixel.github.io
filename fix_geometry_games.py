#!/usr/bin/env python3
"""
Fix all 98 Geometry Practice files:
1. Replace broken game-hub-container + empty game containers with full inline game UI (matching Physics/Bio)
2. Add the "Other games" dropdown menu (Practices-menu)
3. Fix script references: blocks_puzzle.js → block_puzzle.js, remove game_utils.js
4. Fix taskbar.js position (move before </body>, not at top of <body>)
5. Preserve existing flashcard data and page titles
"""
...
# The full working game UI HTML template (from Physics reference)
GAME_UI_TEMPLATE = '''<div class="diagram-card">
...
def get_practices_menu(lesson_id):
    """Generate the Practice-actions footer with 'Other games' dropdown and 'Next Up: Quiz' link."""
    return f'''<div class="Practice-actions" style="margin-top:2rem;display:flex;justify-content:flex-end;width:100%;">
...
<a class="side-button" href="Lesson{lesson_id}_Quiz.html" style="text-align:center; text-decoration:none; display:block;">Next Up: Quiz</a>
</div>'''

def fix_practice_file(filepath):
    """Fix a single Geometry Practice file."""
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()
    
    original = content
    
    # Extract lesson ID (e.g., "1.1" from "Lesson1.1_Practice.html")
    fname = os.path.basename(filepath)
    m = re.search(r'Lesson(\d+\.\d+)_Practice', fname)
    if not m:
        return "SKIP", "Could not extract lesson ID"
    lesson_id = m.group(1)
    
    # Extract the existing flashcard data (preserve it)
    flashcard_match = re.search(
        r'(<script>\s*window\.lessonFlashcards\s*=\s*\[.*?\];\s*</script>)',
        content, re.DOTALL
    )
    if not flashcard_match:
        return "WARN", "No flashcard data found"
    flashcard_script = flashcard_match.group(1)
    
    # Extract page title
    title_match = re.search(r'<h2 class="page-title">(.*?)</h2>', content)
    page_title = title_match.group(1) if title_match else f"Lesson {lesson_id} Practice"
    
    # Remove the taskbar.js from top of body (if present there)
    content = re.sub(r'\s*<script src="../../scripts/taskbar\.js"></script>\s*\n\s*<main', '\n    <main', content)
    
    # Build the new practice-content-view section
    new_practice_view = f'''<div id="practice-content-view">
<h2 class="page-title">{page_title}</h2>
{GAME_UI_TEMPLATE}
{get_practices_menu(lesson_id)}
</div>'''
    
    # Replace the entire practice-content-view
    content = re.sub(
        r'<div id="practice-content-view">.*?</div>\s*(?=</main>)',
        new_practice_view + '\n',
        content,
        flags=re.DOTALL
    )
    
    # Build new script section before </body>
    new_scripts = f'''<!-- ArisEdu Global Search -->
...
<script src="../../scripts/block_puzzle.js"></script>
    ...
    # Remove everything between </main> and </html>, then rebuild
    content = re.sub(
        r'</main>.*?</body>',
        '</main>\n' + new_scripts,
        content,
...
for unit_dir in sorted(glob.glob(os.path.join(BASE, "Unit*"))):
    for filepath in sorted(glob.glob(os.path.join(unit_dir, "Lesson*_Practice.html"))):
        total += 1
        fname = os.path.basename(filepath)
        lesson_match = re.search(r'Lesson(\d+\.\d+)', fname)
        lesson_id = lesson_match.group(1) if lesson_match else fname
        
        try:
            status, msg = fix_practice_file(filepath)
            if status == "OK":
                fixed += 1
                print(f"  OK: Lesson {lesson_id}")
            elif status == "WARN":
                fixed += 1
                print(f"  WARN: Lesson {lesson_id} - {msg}")
            elif status == "UNCHANGED":
                print(f"  UNCHANGED: Lesson {lesson_id}")
            else:
                print(f"  {status}: Lesson {lesson_id} - {msg}")
        except Exception as e:
            errors.append((lesson_id, str(e)))
            print(f"  ERROR: Lesson {lesson_id} - {e}")

print(f"\nDone! Fixed {fixed}/{total} files. Errors: {len(errors)}")
for lid, err in errors:
    print(f"  - Lesson {lid}: {err}")
