...
for filename in files:
    ...
    # 3. Inject Scripts (Smart Update)
    if body := soup.find("body"):
        def update_script(id_val, content, keyword, name):
            global modified
            found = soup.find("script", id=id_val)
            if not found:
                for s in soup.find_all("script"):
                    if s.string and keyword in s.string:
                        found = s
                        found['id'] = id_val
                        break
            
            if found:
                if found.string and found.string.strip() != content.strip():
                    found.string = content
                    modified = True
                    print(f"  - Updated {name} JS")
            else:
                new_s = soup.new_tag("script", id=id_val)
                new_s.string = content
                body.append(new_s)
                modified = True
                print(f"  - Injected {name} JS")

        update_script("script-mixmatch", mixmatch_js, "initMixMatch", "Mix & Match")
        update_script("script-blockpuzzle", blockpuzzle_js, "initBlockPuzzle", "Block Puzzle")
        update_script("script-switcher", switcher_js, "switchToMixMatch", "Switcher")
