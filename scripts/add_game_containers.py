...
# Extract these game containers from a reference file and inject into Algebra2 practice files

GAME_CONTAINERS = '''<div id="climb-game-container" style="display:none; width:100%;"> <div id="climb-game-ui" style="height: 100%; display:flex; flex-direction:column; position:relative; overflow:hidden; background-color: #e2e8f0; border-radius: 1rem; height: 64rem; font-family: 'Orbitron', sans-serif;">
...
def add_game_containers_to_algebra2():
    """Add climb, mixmatch, and blockpuzzle containers to all Algebra2 practice files"""
    ...
    for unit_num in range(1, 10):
        ...
        # Find only Practice files
        lesson_files = [f for f in os.listdir(unit_dir) if 'Practice' in f and f.endswith('.html')]
        ...
        for lesson_file in lesson_files:
            ...
            try:
                ...
                # Check if file already has climb-game-container (if so, skip)
                if 'id="climb-game-container"' in content:
                    print(f"Skipped (already has containers): {lesson_file}")
                    continue
                
                # Find the closing </div> of flashcard-game and insert containers after it
                # Look for pattern: </div>\n</div> (closes flashcard game, closes outer div)
                match = re.search(r'(</div>\n</div>)\s*(<div class="Practice-actions")', content)
                if match:
                    # Insert the game containers between the flashcard game closing and Practice-actions
                    updated_content = content[:match.start(2)] + GAME_CONTAINERS + '\n' + content[match.start(2):]
                    
                    # Write back
                    with open(file_path, 'w', encoding='utf-8') as f:
                        f.write(updated_content)
                    
                    print(f"Added containers to: {lesson_file}")
                    updated_files += 1
                else:
                    print(f"Could not find insertion point in: {lesson_file}")
                    
            except Exception as e:
                print(f"Error processing {lesson_file}: {e}")
    
    print(f"\nSummary: {updated_files} out of {total_files} Practice files updated")

if __name__ == '__main__':
    add_game_containers_to_algebra2()
