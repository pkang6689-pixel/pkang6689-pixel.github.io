#!/usr/bin/env python3
"""
Offline Chinese translation using Hugging Face MarianMT models.
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('translation_progress_chinese.log'),
...
def find_translation_files() -> List[Path]:
    """Find all Chinese translation JSON files."""
    zh_dir = TRANSLATIONS_DIR / LANGUAGE
    if not zh_dir.exists():
        logger.error(f"Translation directory not found: {zh_dir}")
        return []

    files = sorted(zh_dir.glob("**/*.json"))
...
def main():
    logger.info("=" * 60)
    logger.info("Starting OFFLINE Chinese translation (no API key needed)")
    logger.info("Using: Helsinki-NLP/Opus-MT-en-zh model")
    logger.info("=" * 60)
