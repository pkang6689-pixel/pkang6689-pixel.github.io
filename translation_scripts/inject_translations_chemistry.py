...
def main():
    try:
        ...
        if not os.path.exists("../scripts/unique_titles.txt"):
            print("unique_titles.txt not found. Running scan_titles.py...")
            os.system("python3 ../scripts/scan_titles.py > ../scripts/unique_titles.txt")
        
        with open("../scripts/unique_titles.txt", "r") as f:
            lines = f.readlines()
